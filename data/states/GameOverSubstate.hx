...
function postCreate() {
	...
	FlxG.cameras.add(new FlxCamera(), false).bgColor = FlxColor.TRANSPARENT;

	giveAchievement('gameover');
	var gameoverSong = 'gameover_' + PlayState.SONG.meta.name;
	if (achievements.data.exists(gameoverSong))
		giveAchievement(gameoverSong);
}
