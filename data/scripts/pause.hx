...
function postCreate():Void {
	...
	for(k => label in [composerText, charterText]) {
		...
		label.y = 15 + (32 * (k + 4.3));
		FlxTween.tween(label, {alpha: 1, y: label.y + 5}, 0.4, {ease: FlxEase.quartInOut, startDelay: 0.3 * (k + 4.3)});
		add(label);
	}
}